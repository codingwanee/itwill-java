...
public class ArrayExampleApp {
	public static void main(String[] args) {
		...
		for(int nai:age) {
			/*
			if(nai>=10 && nai<=19) cnt[0]++;//cnt[0] : 10대 인원수
			else if(nai>=20 && nai<=29) cnt[1]++;//cnt[1] : 20대 인원수
			else if(nai>=30 && nai<=39) cnt[2]++;//cnt[2] : 30대 인원수
			else if(nai>=40 && nai<=49) cnt[3]++;//cnt[3] : 40대 인원수
			else if(nai>=50 && nai<=59) cnt[4]++;//cnt[4] : 50대 인원수
			else if(nai>=60 && nai<=69) cnt[5]++;//cnt[5] : 60대 인원수
			*/
