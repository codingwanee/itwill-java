...
public class CalculatorFrameApp extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(button==b_equals) {//Equals 버튼을 누른 경우
			...
			//연산자가 존재하지 않을 경우 메소드 종료
			if(index==-1) return;
			
			try {
				//연산자 위치정보를 이용하여 피연산자와 연산자를 분리하여 저장
			...
			//연산식 필드에 대한 초기화 및 출력 컴퍼넌트 초기화
			operation="";
			//JLabel.setText(String text) : 문자열을 전달받아 라벨 컴퍼넌트의 문자열을 변경하는 메소드
			label.setText("0");
