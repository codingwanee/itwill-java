...
public class BaseballApp {
	public static void main(String[] args) {
		...
		//문제에 대한 입력 기회 제공 >> 입력에 대한 결과 출력 
		for(int cnt=1;cnt<=15;cnt++) {
			//스트라이크와 볼의 갯수를 저장하기 위한 변수 선언
			int strike=0, ball=0;
			
			//입력 정수 3개를 저장하기 위한 배열 선언
			int[] num=new int[3];
			...
			while(true) {
				...
				for(int i=0;i<num.length;i++) {
					//문자열에서 첨자위치의 문자를 반환받아 정수로 변환하여 배열 요소에 저장
			...
			for(int i=0;i<dap.length;i++) {//난수값이 저장된 배열요소의 첨자
				for(int j=0;j<num.length;j++) {//입력값이 저장된 배열요소의 첨자
					if(dap[i]==num[j]) {//배열 요소의 난수값과 입력값이 같다면
